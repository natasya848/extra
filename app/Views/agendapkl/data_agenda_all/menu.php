...
if ($query->getNumRows() > 0) {
	...
	if ($level == 3 && $jurusan == 2) {
		header("Location: " . base_url('agendapkl/data_agenda_all/rpl/'));
		exit(); // Pastikan untuk keluar dari script setelah melakukan redirect
	}

	if ($level == 3 && $jurusan == 3) {
		header("Location: " . base_url('agendapkl/data_agenda_all/akl/'));
		exit(); // Pastikan untuk keluar dari script setelah melakukan redirect
	}

	if ($level == 3 && $jurusan == 4) {
		header("Location: " . base_url('agendapkl/data_agenda_all/bdp/'));
		exit(); // Pastikan untuk keluar dari script setelah melakukan redirect
	}
?>

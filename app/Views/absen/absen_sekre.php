        ...
        <div class="card-body">
          <form method="post">
            ...
            <div class="table-responsive">
              ...
              <!-- Hidden input untuk rombel -->
              <input type="hidden" name="rombel" value="<?= $siswaData[0]['id_rombel'] ?? '' ?>">

              <!-- Hidden input untuk blok -->
              <input type="hidden" name="blok" value="<?= $id_blok ?? '' ?>">
  ...
  idSiswaElements.forEach((idSiswaElement, index) => {
    ...
    .then((data) => {
      ...
      if (data.length > 0) {
        ...
        const radioTK = statusRadioButtons[index * 4 + 3];

        if (siswaData.status === "S") {
          radioSakit.checked = true;
        } else if (siswaData.status === "I") {
          radioIzin.checked = true;
        } else if (siswaData.status === "TK") {
          radioTK.checked = true;
        } else {
          radioHadir.checked = true; // "H"
        }
      }
    })
    .catch((error) => {

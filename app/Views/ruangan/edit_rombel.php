    ...
    <div class="page-title">
      <div class="row">
        <div class="col-12 col-md-6 order-md-1 order-last">
          <h3>Edit <?= $title ?></h3>
          <p class="text-subtitle text-muted">Silakan Edit <?= $title ?></p>
        </div>
        ...
        <div class="col-md-6 col-12">
          <div class="card">
            <div class="card-content">
              <div class="card-body">
                <form class="form-horizontal" novalidate action="<?= base_url('aksi_edit_rombel/'.$jojo->id_rombel) ?>" method="post">
                  <?= csrf_field() ?>  
                <input type="hidden" name="id" value="<?= $jojo->id_rombel ?>">
                  <div class="form-body">
                    <div class="row">
                      <div class="col-md-4">
                      ...
                      <div class="col-md-8 form-group">
                        <input type="text" class="form-control" name="nama" placeholder="Nama Rombel" value="<?= $jojo->nama_r ?>" />
                      </div>
                      ...
                      <div class="col-md-8 form-group">
                        <select class="form-select" name="jenjang" id="jenjang" required>
                          ...
                          <?php foreach ($j as $jen) : ?>                            
                            <option value="<?= $jen->id_jenjang ?>" <?= ($jen->id_jenjang == $jojo->jenjang ? 'selected' : '') ?>>
                              <?= $jen->nama_jenjang ?>
                            </option>
                          <?php endforeach; ?>
                        </select>
                      ...
                      <div class="col-md-8 form-group">
                        <select class="form-select" name="kelas" id="kelas">
                          <option value="">-PILIH-</option>
                          <?php foreach ($g as $h) : ?>
                            <option value="<?= $h->id_kelas ?>" data-id_kelas="<?= $h->id_kelas ?>" <?= ($jojo->kelas == $h->id_kelas) ? 'selected' : '' ?>>
                              <?= $h->nama_kelas ?>
                            </option>
                          <?php endforeach; ?>
                        </select>
                      ...
                      <div id="jurusan-wrapper" class="col-md-12 d-flex">
                        <div class="col-md-4">
                          <label>Jurusan</label>
                        </div>
                        <div class="col-md-8 form-group">
                          <select class="form-select" id="jurusan_select">
                            <option value="">-PILIH-</option>
                            <?php foreach ($a as $b) : ?>
                              <?php if ($b->id_jurusan != 5 && $b->id_jurusan != 6) : ?>
                                <option value="<?= $b->id_jurusan ?>" <?= ($jojo->jurusan == $b->id_jurusan) ? 'selected' : '' ?>>
                                  <?= $b->nama_jurusan ?>
                                </option>
                              <?php endif; ?>
                            <?php endforeach; ?>
                      ...
                      <input type="hidden" name="jurusan" id="jurusan" value="<?= $jojo->jurusan ?>">
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateJurusan() {
      ...
      if (jenjangVal === ID_SMK) {
        ...
        jurusanHidden.value = jurusanSelect.value || "5";
    ...
    function filterKelas() {
      ...
      Array.from(kelasSelect.options).forEach(option => {
        ...
        if (jenjangVal === ID_SMK) {
          ...
          if (!found && allowedSMK.includes(idKelas)) found = true;
        } else if (jenjangVal === ID_SMP) {
          option.style.display = allowedSMP.includes(idKelas) ? "block" : "none";
          if (!found && allowedSMP.includes(idKelas)) found = true;
        } else if (jenjangVal === ID_TIDAK_MENGAJAR) {
          option.style.display = idKelas === ID_KELAS_TIDAK_MENGAJAR ? "block" : "none";
          if (!found && idKelas === ID_KELAS_TIDAK_MENGAJAR) found = true;
        } else {
